<template>
  <div class="bg-white rounded-[2.5rem] border border-slate-200 shadow-xs p-8 text-left animate-fade-in">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 border-b border-slate-100 pb-5">
      <div>
        <h3 class="text-xl font-black text-slate-800 uppercase tracking-tight">Indicadores de Calidad y Cumplimiento</h3>
      ...
      <button
        ...
        class="px-5 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-black uppercase tracking-widest transition-all flex items-center gap-2 shadow-2xs border border-slate-200/80 cursor-pointer print:hidden"
      >
        <v-icon name="bi-printer-fill" scale="1.1" class="text-[#005596]" />
        Imprimir Reporte
      </button>
    </div>
    ...
    <div v-if="loading" class="py-12 text-center">
      <div class="animate-spin inline-block w-8 h-8 border-4 border-cyan-600 border-t-transparent rounded-full"></div>
    </div>

    <!-- GRID DE MÉTRICAS -->
    <div v-else-if="auditData" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">

      <div class="bg-slate-50/80 border border-slate-200/80 rounded-3xl p-6 relative overflow-hidden shadow-2xs">
        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Satisfacción General</p>
        <h3 class="text-3xl font-black text-emerald-600 mt-2">{{ auditData.SatisfaccionGeneral }}%</h3>
      ...
      <div class="bg-slate-50/80 border border-slate-200/80 rounded-3xl p-6 relative overflow-hidden shadow-2xs">
        ...
        <h3 class="text-3xl font-black text-[#005596] mt-2">{{ auditData.TiempoEsperaPromedio }} min</h3>
      ...
      <div class="bg-slate-50/80 border border-slate-200/80 rounded-3xl p-6 relative overflow-hidden shadow-2xs">
        ...
        <h3 class="text-3xl font-black text-amber-600 mt-2">{{ Number(auditData.TotalQuejas).toFixed(0) }}</h3>
      ...
      <div class="bg-slate-50/80 border border-slate-200/80 rounded-3xl p-6 relative overflow-hidden shadow-2xs">
        ...
        <h3 :class="Number(auditData.ErroresMedicos) > 0 ? 'text-rose-600' : 'text-slate-400'" class="text-3xl font-black mt-2">
      ...
      <div class="bg-slate-50/80 border border-slate-200/80 rounded-3xl p-6 relative overflow-hidden shadow-2xs">
        ...
        <h3 class="text-3xl font-black text-cyan-600 mt-2">{{ auditData.TasaReadmision }}%</h3>
      ...
      <div class="bg-slate-50/80 border border-slate-200/80 rounded-3xl p-6 relative overflow-hidden shadow-2xs">
        ...
        <h3 class="text-3xl font-black text-orange-600 mt-2">{{ Number(auditData.TasaComplicaciones).toFixed(2) }}%</h3>
    ...
    <!-- ESTADO SIN DATOS -->
    <div v-else class="py-12 text-center text-slate-400 font-bold text-xs uppercase tracking-widest border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50/30">
