  ...
  <div class="min-h-screen bg-[#f8fafc] text-left font-sans">
    ...
    <header class="bg-linear-to-r from-[#005596] to-[#00345c] text-white px-8 py-3 flex justify-between items-center shadow-lg">
      ...
      <div class="flex items-center gap-4">
        ...
        <button @click="handleLogout" class="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-xl text-xs font-bold transition-all border border-white/10 flex items-center gap-2">
          <span>↪</span> Salir
        </button>
      </div>
    </header>

    <!-- MENÚ DE PESTAÑAS BLANCAS -->
      ...
      <div class="max-w-350 mx-auto flex gap-2">
        <button
          ...
          :class="[
            ...
            currentPath === item.path ? 'bg-[#005596] text-white shadow-md shadow-blue-100' : 'text-slate-500 hover:bg-slate-50'
          ]"
        >
